	...
	implements ITurnRepository
{
	...
	private mapToPersistence(turn: Turn): typeof turnsTable.$inferInsert {
		return {
			...
			errorKind: turn.get("state").error?.kind,
			errorMessage: turn.get("state").error?.message,
			errorRetryAfter: turn.get("state").error?.retryAfter,
			failedAt: turn.get("state").failedAt,
			intent: turn.get("intent"),
			previousTurnId: turn.get("previousTurnId")?.value(),
			settledAt: turn.get("state").settledAt,
			status: turn.get("state").get("status"),
			updatedAt: turn.get("updatedAt"),
			usageCompletionTokens: turn.get("usage")?.completionTokens,
			usageCostUsd: turn.get("usage")?.costUsd,
			usagePromptTokens: turn.get("usage")?.promptTokens,
		};
	}
}
