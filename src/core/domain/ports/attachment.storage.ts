...
export interface IAttachmentStorage {
	/**
	 * @description
	 * Permanently deletes all file attachments nested inside a specific room folder.
	 * * Called during the room closure/deletion flow by `DeleteRoomHandler` to
	 * prevent orphaned files in storage. Execution uses admin privileges to bypass RLS.
	 */
