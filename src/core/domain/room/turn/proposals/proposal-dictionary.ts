...
/**
 * @description
 * Rich, contextual microcopy for turn proposal suggestions
 * shown to the moderator. Framed as "what this participant
 * might want to do next" rather than robotic labels.
 */
const PROPOSAL_DICTIONARY = {
	[TurnIntent.RESPOND]: {
		curious: [
...
/**
 * @description
 * Picks an emotional tone for proposal copy based on
...
export function selectMood(context: {
	...
	if (context.hasFailedTurn) {
		return "urgent";
	} else if (
		context.lastIntent === TurnIntent.CHALLENGE ||
		context.lastIntent === TurnIntent.CRITIQUE
	) {
		return "assertive";
	} else if (context.participantCount > 2 && context.turnCount > 5) {
		return "reflective";
	} else if (context.turnCount > 8) {
		return "playful";
	} else {
		return "curious";
	}
