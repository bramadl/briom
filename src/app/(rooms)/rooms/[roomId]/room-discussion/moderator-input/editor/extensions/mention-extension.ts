...
const mentionClass = cn(
	"inline-flex items-center gap-0.5 px-1 py-0.5 rounded-md",
	"bg-primary/15font-medium text-xs text-primary select-none",
);
...
export function buildMentionExtension(
	...
	return Mention.configure({
		...
		renderHTML({ node }) {
			return [
				"span",
				{ class: mentionClass, "data-mention-id": node.attrs.id },
